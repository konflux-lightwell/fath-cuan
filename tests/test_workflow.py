...
def test_process_osv_returns_list(mock_fetch: object) -> None:
...
@patch("fath_cuan.converters.osv._fetch_upstream_osv", return_value=None)
def test_process_osv_has_correct_id(mock_fetch: object) -> None:
    result = process_osv(SAMPLE_INPUT_DATA)
    assert result[0]["id"] == "x_RHLW-CVE-2024-25710-1.0.0"


@patch("fath_cuan.converters.osv._fetch_upstream_osv", return_value=None)
def test_process_osv_has_schema_version(mock_fetch: object) -> None:
    result = process_osv(SAMPLE_INPUT_DATA)
    assert result[0]["schema_version"] == "1.6.8"


@patch("fath_cuan.converters.osv._fetch_upstream_osv", return_value=None)
def test_process_osv_has_aliases(mock_fetch: object) -> None:
    result = process_osv(SAMPLE_INPUT_DATA)
    assert result[0]["aliases"] == ["CVE-2024-25710"]


@patch("fath_cuan.converters.osv._fetch_upstream_osv", return_value=None)
def test_process_osv_excludes_none_values(mock_fetch: object) -> None:
    result = process_osv(SAMPLE_INPUT_DATA)
